...
namespace Butterfly.OpenTracing.Dispatcher
{
    internal class TimerDispatchHandler : IDisposable
    {
        ...
        public TimerDispatchHandler(IEnumerable<IDispatchCallback> callbacks, ILoggerFactory loggerFactory, int flushInterval)
        {
            ...
            _timer = new Timer(async _ => await FlushCallback(DateTimeOffset.UtcNow).ConfigureAwait(false),
                                null, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(_flushInterval));
        }
            ...
            foreach (var tokendState in tokendStates)
            {
                ...
                try
                {
                    foreach (var callback in _callbacks)
                    {
                        ...
                        {
                            await callback.Accept(states).ConfigureAwait(false);
                        }
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    _logger.LogError(new EventId(1), exception, "Flush data to collector error.");
                }
                finally
                {
